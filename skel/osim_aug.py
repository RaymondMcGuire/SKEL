...
class OsimAug():
    ...
    def __init__(self, osim_path=cg.osim_model_path) -> None:
        ...
        self.node_names = [n.getName()
                           for n in self.osim.skeleton.getBodyNodes()]
        self._meshes_dict = {}
        self._indices_dict = {}
    ...
    def get_markers_location(self):
        ...
        markers_abs = self.skeleton.getMarkerMapWorldPositions(
            self.osim.markersMap)
        print(f"Number of markers: {len(markers_abs)}")

        markers_pos_rel = np.vstack(
            [m[1] for m in self.osim.markersMap.values()])
    ...
    def _generate_meshes_dict(self):
        ...
        for node_name in node_names:
            ...
            if mesh_list:
                ...
                self.indices_dict[node_name] = (
                    current_index, current_index + node_mesh.vertices.shape[0])
    ...
    def visualize(self):
        ...
        osim_model_results = OSIMSequence.a_pose(
            osim_path=self.osim_path, name='result_osim', color_markers_per_part=True, color_skeleton_per_part=True)
