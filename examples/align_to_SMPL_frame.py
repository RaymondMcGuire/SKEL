...
if __name__ == '__main__':
    ...
    parser.add_argument('--smpl_mesh_path', type=str,
                        help='Path to the SMPL mesh to align to', default=None)
    parser.add_argument('--smpl_data_path', type=str,
                        help='Path to the SMPL dictionary to align to (.pkl or .npz)', default=None)
    parser.add_argument('-o', '--out_dir', type=str,
                        help='Output directory', default='output')
    parser.add_argument('-F', '--force-recompute',
                        help='Force recomputation of the alignment', action='store_true')
    parser.add_argument(
        '--gender', type=str, help='Gender of the subject (only needed if not provided with smpl_data_path)', default='female')
    ...
    smpl_data = load_smpl_seq(
        args.smpl_data_path, gender=args.gender, straighten_hands=False)
    ...
    if args.smpl_mesh_path is not None:
        ...
        raise ValueError(
            'Either smpl_mesh_path or smpl_data_path must be provided')
    ...
    SKEL_skin_mesh = trimesh.Trimesh(
        vertices=skel_seq['skin_v'][0], faces=skel_seq['skin_f'])
    SKEL_skel_mesh = trimesh.Trimesh(
        vertices=skel_seq['skel_v'][0], faces=skel_seq['skel_f'])
    SMPL_mesh = trimesh.Trimesh(
        vertices=skel_seq['smpl_v'][0], faces=skel_seq['smpl_f'])
    ...
    print('SKEL skin mesh saved to {}'.format(
        os.path.join(subj_dir, subj_name + '_skin.obj')))
    print('SKEL skel mesh saved to {}'.format(
        os.path.join(subj_dir, subj_name + '_skel.obj')))
    print('SMPL mesh saved to {}'.format(
        os.path.join(subj_dir, subj_name + '_smpl.obj')))
